#!/usr/bin/env bash
# AgentData relational connector — Linux installer.
#
# Prerequisites (one-time):
#   • Python 3.10+ + venv  →  Debian/Ubuntu: sudo apt install python3 python3-venv python3-pip
#                              RHEL/Fedora:   sudo dnf install python3 python3-pip
#   • For SQL Server only: install the Microsoft ODBC driver (msodbcsql18) + unixODBC
...
if ! command -v "$PY" >/dev/null 2>&1; then
  echo "ERROR: python3 not found. Install Python 3.10+ and re-run (see header for your distro)." >&2
  exit 1
fi
...
if [ ! -f .env ]; then
  ...
  echo "     then re-run ./install-linux.sh"
  exit 0
fi
